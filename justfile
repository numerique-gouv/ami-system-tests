...
test-android tag="": android-start
    #!/usr/bin/env bash
    set -euo pipefail
...
# Usage : just test-android-builded [tag]
test-android-builded tag="": build-android android-start
    #!/usr/bin/env bash
    set -euo pipefail
    if [ -n "{{tag}}" ]; then
        npm run test:android -- --mochaOpts.grep "{{tag}}"
...
# Usage : just test-ios-builded [tag]              → avec reset (défaut)
#         just test-ios-builded Notifications reset=""  → sans nettoyage (debug)
test-ios-builded tag="" reset="true": build-ios ios-start
    #!/usr/bin/env bash
    set -euo pipefail
    if [ -n "{{reset}}" ]; then
        just _ios-reset-fc-session
...
inspect hash="":
    ...
    if [ -n "$ANDROID_DEVICE" ] && [ -n "$IOS_DEVICE" ]; then
        echo "❌ Android ($ANDROID_DEVICE) ET iOS ($IOS_DEVICE) détectés. Arrête-en un avec 'just android-stop' ou 'just ios-stop'."
        exit 1
    fi
    if [ -z "$ANDROID_DEVICE" ] && [ -z "$IOS_DEVICE" ]; then
        echo "❌ Aucun appareil détecté. Lance 'just android-start' ou 'just ios-start'."
        exit 1
    fi
